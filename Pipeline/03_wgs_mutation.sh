...
echo "========================================================"
echo "Starting Part 3: WGS Mutation Calling Pipeline"
echo "========================================================"
mkdir -p "$WGS_OUT_ROOT"

for spl in "${SAMPLES[@]}"; do
    echo "----------------------------------------"
    echo "[WGS Mutect2] Sample: $spl"
    bamfile="$DIR/$spl/${spl}.BQSR.bam"
    outdir="$WGS_OUT_ROOT/$spl"
    mkdir -p "$outdir"
    ...
    echo "[1/6] Mutect2 Calling (Whole Genome)..."
